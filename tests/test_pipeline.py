...
from eva02_classification_pipeline import (
    DEFAULT_WEIGHTS_DIR,
    MAX_BATCH,
    MAX_IMAGE_SIDE,
    MODEL_ID,
...
from eva02_classification_pipeline import pipeline as pipeline_module
...
def _pipeline() -> EVA02ClassificationPipeline:
    labels = tuple(f"class-{i}" for i in range(NUM_CLASSES))
    return EVA02ClassificationPipeline(_fake_runner, _fake_transform, "cpu", labels, "injected")
...
def test_identity_constants_are_40_hex_and_named():
    ...
    assert MODEL_ID == "timm/eva02_base_patch14_448.mim_in22k_ft_in22k_in1k"
...
def test_from_pretrained_refuses_without_snapshot_or_download(tmp_path: Path):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        EVA02ClassificationPipeline.from_pretrained(weights_dir=tmp_path, allow_download=False)
...
def test_predict_fine_tuned_reloaded_artifact_default_top_k(tmp_path: Path) -> None:
    ...
    pipe = EVA02ClassificationPipeline.from_pretrained(weights_dir=tmp_path)
...
def test_fit_fails_closed_missing_base_weights(tmp_path: Path) -> None:
    ...
    with pytest.raises(FileNotFoundError):
        EVA02ClassificationPipeline.fit(
