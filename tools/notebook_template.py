...
TEMPLATE = {
    ...
    "learning_objectives": (
        ...
        "correctly, execute 100% in-kernel fine-tuning on custom classes with AdamW and cross-entropy, export and "
        "fresh-reload fine-tuned artifacts, exercise an optional BYOD path, produce an evaluation report that is `sample-sanity` "
        "only when a ground-truth class index exists and `not-measurable` otherwise, and export machine-readable outputs plus provenance."
    ),
    "exclusions": (
        "object detection, segmentation, multi-label tagging, OCR, open-vocabulary classification, feature/embedding "
        "extraction (the DINOv2 sibling covers that). The base label space is fixed to the 1000 ImageNet-1k "
        "classes; an image whose subject is outside that space still receives a label unless adapted via in-kernel fine-tuning."
    ),
    "prerequisites": [
    ...
    "cells": [
        ...
        },
        {
            "md": (
                ...
                "container), `EVA02ClassificationPipeline.from_pretrained` loads the newly generated `model.safetensors` "
        ...
        },
        {
            "md": (
                ...
                "the evaluation report, the fine-tuning training history and in-notebook evaluation metrics, the sample identity, "
                "aspect ratio and digest, the notebook's source (repository, revision, embedded module digest, generator), the model "
                "identifier, the immutable model revision, the model licence, and the runtime identity (Python, `torch`, `timm`, device). "
                "The rank-ordered top-k table and fine-tuned validation predictions are written as CSV files so class ordering survives "
                "downstream use. Deployable fine-tuned model artifacts and evaluation reports are published in `outputs/{stem}_finetuned/` "
                "and `outputs/`. No credentials are recorded."
            ),
            "code": (
                "import csv\n\n"
                "payload = {{\n"
    ...
    "closing": (
        ...
        "BYOD image is 0 or 1 and says nothing about the error rate on a domain. Every input is squash-resized to 448 x 448, "
        "so strongly non-square subjects are distorted before classification; the pipeline does not detect "
        "out-of-distribution inputs (drawings, scans, satellite tiles), blur, or capture-device drift, and it exposes "
        "no features, no detection, and no labels beyond the fixed 1000.\n\n"
        ...
        "execute the public pipeline path, perform in-kernel fine-tuning, reload the verified artifact bundle, and emit "
        "the shown machine-readable outputs in the tested runtime — without the repository being reachable. It does **not** "
        "establish benchmark superiority, reproduction of the upstream accuracy, deployment calibration, safety for "
        ...
        "there. A very slow Section 6 or 8 on a CPU runtime is expected for this 107-GMAC model; switch to a CUDA runtime or "
        ...
        "to `pipe.fit` to adapt to your domain; upload a strongly non-square version of the same photograph to observe the "
        "effect of the squash; compare the CUDA and CPU top-5 orderings on the same image to observe kernel-level variability.\n\n"
        "## References\n\n"
